...
> {
  ...
  constructor(
    ...
    this._prefix = this.config.importOperationTypesFrom
  ...
  private _buildComponent(
    ...
    const propsType = `$Diff<ApolloReactComponents.${operationType}ComponentOptions<${operationResultType}, ${operationVariablesTypes}>, {| ${operationType.toLowerCase()}: * |}>`
  ...
  private _buildHooks(
    ...
    const hookResults = [
      `export type ${operationName}HookResult = ExtractReturnType<typeof use${operationName}>;`
    ]
    ...
    if (operationType === 'Query') {
      ...
      hookResults.push(
        `export type ${lazyOperationName}HookResult = ExtractReturnType<typeof use${lazyOperationName}>;`
      )
    }
